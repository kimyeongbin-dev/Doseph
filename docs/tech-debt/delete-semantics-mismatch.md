...
| 상태 | **✅ 두 건 모두 해소 (2026-09-15)** — QA-02 `a84b227` / QA-01 S1~S7 (선택지 **A 확장판**: soft delete 전면 폐지). 후속 **QA-29(단계적 삭제)도 1회차 완료** — 배치 7일 유예 + 가이드 삭제 영향 고지. **탈퇴 유예는 미착수** (정본 = `docs-private/TEST_FOLLOWUP_QUEUE.md`) |
|---|---|
...
**아직 남은 것**: **탈퇴(`delete_account`)는 여전히 유예 0일**이다. 계정·세션·재가입
경로가 얽혀 범위가 커서 별도 회차로 미뤘다. tombstone 도 지금은 **로그**라 회전하면
사라진다 — 테이블화는 `QA-31`.
