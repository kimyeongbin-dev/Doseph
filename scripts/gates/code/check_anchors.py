...
- 안 잡는다: 큐에 있는데 코드에 앵커가 없는 경우 → **정상이다.** 미착수 항목은
  아직 코드에 손대지 않았으므로 앵커가 없는 게 당연하다. 그건 게이트가 아니라
  ``build_followup_index.py`` 가 *열림* 으로 세어 보고한다.

  .. note::

     🔴 **2026-09-23 정정**: 이 자리는 원래 ``defect_map.py`` 를 가리키고 있었는데
     **그 파일은 존재한 적이 없다**(git 히스토리 0건). 중단된
     ``plan/2026-09-16_stabilization-plan.md`` 의 **S4 계획**이었다.
     *"만들 것"* 을 *"있는 것"* 처럼 인용한 주석이라, 읽는 사람을 없는 도구로 보냈다.
     — 대장 ``D13``(소스를 안 읽고 도구 동작을 단정하지 않는다).
