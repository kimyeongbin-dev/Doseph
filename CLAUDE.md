...
## 6-4. 🔴 "지금 닫아도 잃을 게 없다" 는 **명령으로만** 말한다

`/clear`·`/compact` 를 제안하기 전에 **반드시** 실행한다:

```bash
uv run python scripts/check_session_close.py
```

**기억으로 판단하지 않는다. 즉석 `grep` 으로도 판단하지 않는다.**
급조한 정규식은 정본 검증기보다 **덜 엄밀한 두 번째 구현**이다(대장 D31).
이 검사기는 규칙을 다시 구현하지 않고 **이미 있는 게이트를 그대로 호출**한다.

| 기계가 판정 (정본이 답한다) | 기계가 판정 못 함 (사람이 답한다) |
|---|---|
| git — 미커밋 · **미푸시** · **stash** (`git status` 는 stash 를 안 보여준다) | 이번 세션의 결정·합의가 파일에 적혔는가 |
| 앵커 · 폐기 어휘 · PLAN 아카이브 게이트 | 발견한 결함이 등재됐는가 |
| 진행 중 PLAN 에 **"진행 현황" 절**이 있는가 (이어갈 지점) | 새 실수가 대장에 등재됐는가 · 배운 개념이 `study/` 에 남았는가 |

> ⚠️ **오른쪽 칸에 `grep` 을 붙여 자동 판정하지 않는다.** `/clear` 가 지우는 것은
> 대화 컨텍스트뿐이고 파일은 남는다. 잃는 것은 *"파일에 없고 내 머릿속에만 있는 것"* 인데
> **그게 무엇인지는 기계가 열거할 수 없다.** 자동 판정하는 순간 거짓 안심이 된다.

**왜 이 규칙이 생겼나**: 2026-09-15, *"지금 지워도 잃을 게 없습니다"* 라고 말했는데
사용자가 되묻자 세어 보니 **5개가 안 적혀 있었다**(PLAN 상태줄 · 진행 현황 절 ·
`CLAUDE.md` 규칙 2개 · 대장 D34). 검사기를 만들어 돌리자 그 즉시 **미푸시 커밋 1건**도
잡혔다 — 나는 "미커밋 0" 만 보고 넘겼었다.
